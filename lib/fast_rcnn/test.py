...
# Returns the array of bounding boxes per classification per image.
# It's a copy of test_net function but without visualizations.
def get_bounding_boxes(sess, net, imdb, weights_filename , max_per_image=300, thresh=0.05):
    """Test a Fast R-CNN network on an image database."""
    num_images = len(imdb.image_index)
    # all detections are collected into:
    #    all_boxes[cls][image] = N x 5 array of detections in
